...
<template>
  <div class="space-y-6">
    <!-- Header Section -->
    <div class="space-y-2">
      <TypographyH1 class="text-2xl md:text-3xl font-bold text-foreground">Dashboard Overview</TypographyH1>
      <TypographyP class="text-muted-foreground">Welcome back! Here's what's happening with your platform today.</TypographyP>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
      ...
      >
        ...
        >
          ...
          <Building2 v-else-if="stat.iconType === 'properties'" class="h-7 w-7" />
          <Clock v-else-if="stat.iconType === 'pending'" class="h-7 w-7" />
          <CircleDollarSign v-else-if="stat.iconType === 'revenue'" class="h-7 w-7" />
        </div>
      </div>
    </div>

    <!-- Business Owners Table -->
    <div class="bg-card rounded-xl border border-border shadow-md hover:shadow-lg transition-shadow duration-200">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between p-6 border-b border-border gap-3">
        <div class="space-y-1">
          <TypographyH2 class="text-lg font-semibold text-foreground">Recent Business Owners</TypographyH2>
          <TypographyH2 class="text-xs text-muted-foreground">Monitor and manage business owner accounts</TypographyH2>
        </div>
      ...
      <Table>
        ...
        <TableBody>
          ...
          >
            <TableCell class="py-4">
              <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col">
                  ...
                  <span class="text-s text-muted-foreground">{{ owner.email }}</span>
                </div>
              </div>
            </TableCell>
            <TableCell class="font-medium text-foreground">{{ owner.businessName }}</TableCell>
            <TableCell>
              <span class="px-2.5 py-1 bg-prof-navy text-foreground text-xs font-semibold rounded-md">
                {{ owner.properties }}
              </span>
            </TableCell>
            <TableCell>
              <span
                ...
                :class="{
                  'bg-success/10 text-success dark:bg-success/20': owner.status === 'active',
                  'bg-warning/10 text-warning dark:bg-warning/20': owner.status === 'pending',
                  'bg-destructive/10 text-destructive dark:bg-destructive/20': owner.status === 'inactive',
                }"
              >
                <span 
            ...
            <TableCell class="text-foreground">{{ owner.joinedDate }}</TableCell>
            <TableCell class="text-right">
              <div class="flex gap-2 justify-end">
                <Button variant="secondary" size="sm" class="hover:scale-105 transition-transform duration-200">
                  View
                </Button>
                <Button variant="default" size="sm" class="hover:scale-105 transition-transform duration-200">
                  Edit
                </Button>
              </div>
            </TableCell>
